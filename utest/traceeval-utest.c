...
/*
 * Create a struct traceeval instance with key_types and val_types fields
 * of just TRACEEVAL_TYPE_NUMBER.
 */
static struct traceeval *create_num_teval(void)
{
	const struct traceeval_type test_data[] =  {
		{
			.type = TRACEEVAL_TYPE_NUMBER,
	...
	return traceeval_init(test_data, test_data);
}

/*
 * Test traceeval_insert(), traceeval_query(), and traceeval_results_release()
 * with valid data.
 */
static void test_i_q_some(void)
{
	/* set up */
	union traceeval_data *results;
	struct traceeval *teval = create_num_teval();
	const union traceeval_data data[] = {
		{
			.number = 1
		}
	};
	const union traceeval_data vals_update[] = {
		{
			.number = 2
		}
	};
	...
	/* test insert new */
	CU_ASSERT(traceeval_insert(teval, data, data) == 0);

	/* test query new */
	CU_ASSERT(traceeval_query(teval, data, &results) == 1);
	CU_ASSERT(results != NULL);
	/* release query */
	traceeval_results_release(teval, results);

	/* test insert update */
	CU_ASSERT(traceeval_insert(teval, data, vals_update) == 0);

	/* test query update */
	CU_ASSERT(traceeval_query(teval, data, &results) == 1);
	CU_ASSERT(results != NULL);
	CU_ASSERT(results->number == vals_update->number);
...
/*
 * Tests the traceeval_insert(), traceeval_query(), and
 * traceeval_results_release() functions.
 */
static void test_i_q(void)
{
	// TODO run valgrind to ensure all data actually freed
	test_i_q_some();
}
...
void test_traceeval_lib(void)
{
	...
	CU_add_test(suite, "Test traceeval insert and query", test_i_q);
}
